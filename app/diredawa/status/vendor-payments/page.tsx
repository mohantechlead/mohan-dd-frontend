...
export default function VendorPaymentStatusPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allRows: VendorPayment[] = Array.isArray(data) ? data : [];
        const byPurchase = new Map<string, VendorPayment[]>();
        ...
        const eligible: VendorPayment[] = [];
        for (const [, payments] of byPurchase) {
          ...
          eligible.push(sorted[sorted.length - 1]);
        }
        setRows(eligible);
      } catch {
  ...
  const openPopup = (paymentNumber: string, nextAction: "completed" | "cancelled") => {
    setSelectedNumber(paymentNumber);
  ...
  const handleSubmit = async () => {
    ...
    if (action === "completed" && !referenceNumber.trim()) {
      showToast({ title: "Reference number required", description: "Please provide a reference number for completed vendor payment.", variant: "error" });
      return;
    }
    if (action === "cancelled" && !remark.trim()) {
      showToast({ title: "Remark required", description: "Please provide a remark when cancelling vendor payment.", variant: "error" });
      return;
    }

    try {
      setBusy(true);
      const res = await fetch(`/api/accounting/vendor-payments/${encodeURIComponent(selectedNumber)}/update-status`, {
        method: "POST",
      ...
      setRows((prev) => prev.filter((x) => x.payment_number !== selectedNumber));
      showToast({ title: `Vendor payment ${action}`, variant: "success" });
      setOpen(false);
  ...
  return (
    <div className="max-w-5xl mx-auto mt-6 space-y-4">
      <h1 className="text-2xl font-bold">Update Vendor Payment Status</h1>
      <div className="border rounded-md overflow-hidden bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} className="px-4 py-4 text-center">Loading eligible vendor payments...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-4 text-center">No vendor payments where all installments are approved.</td></tr>
            ) : (
              rows.map((x) => (
                <tr key={x.id} className="border-t">
                  <td className="px-4 py-2">{x.purchase_number}</td>
                  <td className="px-4 py-2">{`Last Payment ${x.installment_number ?? 1}`}</td>
                  <td className="px-4 py-2 capitalize">{x.status}</td>
                  <td className="px-4 py-2 flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => router.push(`/diredawa/accounting/vendor-payments/purchase/${encodeURIComponent(x.purchase_number)}`)}><Eye className="w-4 h-4" /></Button>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={busy} style={action === "completed" ? { backgroundColor: "#16a34a", color: "white" } : undefined}>
              {busy ? "Updating..." : action === "completed" ? "Complete" : "Cancel Vendor Payment"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
