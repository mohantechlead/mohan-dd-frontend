...
export default function EditReceivedPaymentPage() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        const data = (await res.json()) as ReceivedPayment;
        if (!res.ok) {
          showToast({ title: "Failed to load received payment", description: (data as any)?.detail || "Please try again.", variant: "error" });
          return;
        }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const res = await fetch(`/api/accounting/received-payments/${encodeURIComponent(params.paymentNumber)}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        showToast({ title: "Failed to update received payment", description: data?.detail || "Please try again.", variant: "error" });
        return;
      }
      showToast({ title: "Received payment updated", variant: "success" });
      router.push(`/diredawa/accounting/received-payments/${params.paymentNumber}`);
    } catch {
      showToast({ title: "Failed to update received payment", description: "Something went wrong.", variant: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto mt-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-2xl font-bold flex-1 text-center">Edit Received Payment</h1>
      ...
      {loading ? (
        ...
        <form onSubmit={handleSave} className="bg-white border rounded-md p-6 space-y-4 text-sm">
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => router.push(`/diredawa/accounting/received-payments/${params.paymentNumber}`)}>Cancel</Button>
