...
export default function CompletedVendorPaymentsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setRows((Array.isArray(data) ? data : []).filter((x: VendorPayment) => x.status === "completed"));
      } catch {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-4 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <h1 className="text-2xl font-bold text-center flex-1">Completed Vendor Payments</h1>
      </div>
      <div className="border rounded-md overflow-hidden bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="px-4 py-4 text-center">Loading completed vendor payments...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-4 text-center">No completed vendor payments found.</td></tr>
            ) : (
              rows.map((x) => (
